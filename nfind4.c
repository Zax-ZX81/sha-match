...
//printf (",%c", filter_list [filter_index].filepath[strlen (filter_list [filter_index].filepath) - 1]);
...
//printf ("DE>%s<\n", dir_ents->d_name);
		...
		if (filtering > 0)				// are we applying filtering?
			...
			for (filter_index = 0; filter_index < filter_line_count; filter_index ++)		// cycle through filter list
				{
				if (!strcmp (filter_list [filter_index].filepath, dir_ents->d_name) && \
...
//printf ("1W%dR%d\tDT=%d\tOT=%c\t%s\t<%s>\n", find_list_write, find_list_read, dir_ents->d_type, find_list [find_list_write].object_type, find_list [find_list_write].filepath, path_sub);
...
//	printf ("2W%dR%d\tDT=%d\tOT=%c\t%s\n", find_list_write, find_list_read, dir_ents->d_type, find_list [find_list_write].object_type, find_list [find_list_write].filepath);
...
//		find_list [find_list_read].filepath [strlen (find_list [find_list_read].filepath) - 1] = NULL_TERM;
...
//---
				...
				if (filtering > 0)				// are we applying filtering?
					...
					for (filter_index = 0; filter_index < filter_line_count; filter_index ++)		// cycle through filter list
						{
printf ("FP=%s\tDN=%s\n", filter_list [filter_index].filepath, dir_ents->d_name);
						if (!strcmp (filter_list [filter_index].filepath, dir_ents->d_name) && \
							(filter_list [filter_index].object_type == T_FIL || filter_list [filter_index].object_type == T_DIR))		// match found
							{
							filter_match = TRUE;
...
//printf ("FLW=%d\tFLC=%d\n", find_list_write, filter_line_count);

for (filter_index = 0; filter_index < find_list_write; filter_index ++)
	{
	if (find_list [filter_index].object_type != T_REJ)
		{
		printf ("%c %s\n", find_list [filter_index].object_type, find_list [filter_index].filepath);
		}
	}
