...
#define SEARCH_LIST_INITIAL_SIZE 4096
#define SEARCH_LIST_INCREMENT 4096
#define SEARCH_LIST_CEILING 262144
...
int searchlist_alloc_size = SEARCH_LIST_INITIAL_SIZE;
int line_index = 0;
...
do
	...
	if (searchlist_lines + 1 == searchlist_alloc_size)		// check memory usage, reallocate
		{
		searchlist_alloc_size += SEARCH_LIST_INCREMENT;
...
start_time = spec.tv_nsec;

clock_gettime(CLOCK_REALTIME, &spec);
printf("Time: %f", (spec.tv_nsec - (float)start_time) / 1000000.0);
